...
int sumVersions(const string &, int &sum, int start = 0);
...
int sumVersions(const string &transmission, int &sum, int start)
{
    ...
    if (type == 4)
    {
        for (int i = start + 6; i < transmission.size(); i += 5)
            if (transmission[i] == '0') return i + 5;
    }
    ...
    if (length_type)
    {
        ...
        for (int i = 0; i < number; ++i)
            next = sumVersions(transmission, sum, next);
        return next;
    }
